...
def _load_roster(cache_key: float | None) -> pd.DataFrame:
    """파티션 전체 로드. cache_key = 파일 mtime — 재수집하면 자동 무효화."""
    return moi_store.load_roster()


def render_outlook(cx: float, cy: float) -> None:
    freshness = moi_store.freshness()
    roster = _load_roster(freshness.timestamp() if freshness else None)
