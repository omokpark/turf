...
아직 안 한 것:
- timeline/ 모듈(개폐업 시계열) 전혀 착수 안 함 — license_fetcher.py, trend.py는 CLAUDE.md 계획에만 존재. 착수하면 지도 탭에 "최근 개업/폐업" 레이어, 세 번째 탭 "변화(시계열)" 추가 예정.
- 유동인구 등 외부 데이터 탭(서울시 상권분석서비스 API 후보), 밀도 정규화·다양성 지수 등 추가 지표.
- GitHub Actions 등 배포/자동화 (5장 "실행 환경" 항목의 "안정화 후" 단계).

⚠️ Day 7~8 변경분(app.py 재작성 2회, analyzer/terrain.py·collector/shop_fetcher.py 버그 수정, .claude/launch.json)이 커밋 안 되어 있을 수 있다. 새 PC에서 이어서 하기 전에 `git status`로 확인하고 커밋·푸시부터 할 것.