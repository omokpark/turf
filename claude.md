...
(2026-07-12 저녁 완료) 휴게음식점 활용신청 승인 → 강남구 수집 완료: 17,162행(영업중 3,907), data/cache/moi/휴게음식점/3220000.parquet. 검증 — 영업중 휴게 125곳(강남역 400m) 전부 affinity 0, 전환 벡터에 커피숍→기타(0→1) 등 휴게발 전환 관측 시작, 전환율 분모 85곳으로 확장, 국면이 3종 기준 '수축'→휴게 포함 '정체'로 변화(커버리지가 판정을 바꿈). 캐시 토큰이 파티션 추가를 자동 감지하므로 서버 재시작 불요. ⚠️ 활용신청 승인 직후 게이트웨이 전파가 불안정(200/403 혼재, ~10분) — 수집이 403으로 죽으면 몇 분 후 재실행.

Day 16 (2단계 피드백 수집 — 방문가치 피드백 + 업소 속성 채널 분리 + 노출 로그, 이번 세션에서 진행):
- **`datasources/feedback_store.py` 신규**: JSON Lines append-only 저장소, 세 갈래로 분리(2026-07-12 설계 합의 그대로 구현).
  - `record_feedback`/`latest_feedback`: 👍(LABEL_UP)/👎(LABEL_DOWN) 방문가치 피드백. **그 순간의 신호값 벡터·점수·배지·스코어러·좌표·반경을 통째로 스냅샷 저장** — 신호는 매일 흘러가므로(개업 경과일 등) 나중에 "그때 그 신호 조합이 틀렸다"로 학습하려면 그 시점 값이 고정돼 있어야 한다. 같은 업소를 다시 평가하면 마지막 것이 이긴다(latest_feedback는 last-wins reduce).
  - `record_attribute`/`latest_attributes`: 폐업했어요/우리 거래처예요 — **방문가치 채널과 절대 분리**. 안 그러면 "이미 거래처라 없었음"이 랭킹 품질 피드백으로 학습돼, 모델이 최고의 개척 후보(거래처와 닮은 집)를 피하도록 학습하게 된다(사용자 지적, 이번 세션 설계의 핵심).
  - `log_exposure`: 상위 30곳 노출 스냅샷. 피드백이 없어도 "무엇이 노출됐는지"가 있어야 나중에 배지별 lift(피드백군 비율 vs 노출 전체 비율)를 계산할 대조군이 생긴다. 키 = (날짜, 구역, 스코어러) — 같은 키는 덮어써 rerun마다 중복 누적되지 않는다.
  - `_to_jsonable`: numpy 스칼라·NaN·Timestamp를 json.dumps가 삼킬 수 있게 변환 — 신호값이 대부분 np.float64/np.int64라 이게 없으면 그대로 죽는다.
- **ui/pages/ranking.py 배선**: 카드마다 👍/👎 버튼(현재 라벨은 `type="primary"`로 강조 — 클릭 후 st.rerun()으로 즉시 반영) + "⋯" 팝오버(체크박스 2개, 값이 바뀔 때만 기록+rerun) + 속성이 있으면 배지 줄과 별도로 칩 표시("🏷️ 우리 거래처" 등 — CSV 내보내기용 "근거" 텍스트와는 분리해 오염 방지). 신호값 스냅샷은 `signal_results`를 업소당 반복 set_index하지 않도록 한 번만 인덱싱해 재사용.
- **함정 기록 (브라우저 검증 중 발견)**: Streamlit의 `st.popover` 콘텐츠는 열려 있을 때 **문서 맨 끝에 포털로 렌더링**된다 — 카드 안 DOM 순서로 좌표 클릭하면 엉뚱한(또는 이미 닫힌) 팝오버를 건드린다. accessibility 트리에서 열린 팝오버의 체크박스를 찾으려면 페이지 트리 최상위 레벨 끝부분을 봐야 한다(ref_id로 좁혀도 안 잡힘 — 카드 컨테이너 하위가 아니므로). read_page(filter=all, 큰 max_chars)로 전체를 봐야 잡힌다.
- 검증: pytest 123 그린(신규 tests/test_feedback_store.py 10개 — last-wins, 노출 중복방지, 잘못된 라벨/속성 거부, numpy/NaN 직렬화). 브라우저 실측 — 👍 클릭 시 신호값 9개(recent_opening·buzz_momentum·franchise·growth_momentum·liquor_adjacency·night_index·review_momentum·star_level·survivor) 전체가 스냅샷에 정확히 남고 버튼이 빨간색으로 강조됨, 거래처 체크박스 클릭 시 attribute_events.jsonl에 기록되고 카드에 "🏷️ 우리 거래처" 칩이 별도로 렌더됨, 노출 로그가 스코어러 전환 시 30곳 스냅샷을 올바로 남김. 검증용 클릭으로 생성된 테스트 데이터는 정리함(data/cache/feedback/ 삭제).

Day 16 후반 (담당구역 즐겨찾기, 같은 세션에서 진행):
- **"우편번호 기준 저장 가능한가?" 검토**: VWorld 주소 API가 우편번호 입력 자체를 못 받음(직접 호출로 확인, "06018" → None). 우편번호 검색이 되는 공식 API(주소기반산업지원서비스, business.juso.go.kr)는 있으나 지금 쓰는 data.go.kr·VWorld 키와 무관한 별도 회원가입·승인이 필요(휴게음식점 API와 같은 종류의 마찰). 결정적으로 새 도로명 우편번호(2015~)는 건물~짧은 도로 구간 단위라, 강남 같은 밀집지역에서 대략 계산하면 반경 150~250m 정도 — **지금 앱의 도보 반경(100~400m)과 비슷하거나 더 작고, 영업사원의 실제 "담당구역"(여러 동 규모)보다 훨씬 작다.** 우편번호 1개 = 구역이 아니라 구역 안의 작은 조각.
- **채택안**: 새 API 없이, 기존 동 이름 검색(이미 동작)으로 좌표를 잡고 **자유 텍스트 이름으로 저장**하는 즐겨찾기 기능. 이름 칸에 우편번호를 적고 싶으면 그렇게 적어도 되지만(라벨일 뿐), 실제 위치 이동은 기존 주소/동 검색이 담당 — 새 지오코딩 연동 없이 "우편번호로 구역을 통보받는다"는 조직상 요구를 라벨로 흡수.
- **`datasources/favorites_store.py` 신규**: `data/cache/favorites.json`에 {이름: {cx, cy, 저장시각}} — 이름이 키라 같은 이름 재저장 = 위치 갱신(덮어쓰기), 별도 "수정" UI 불필요.
- **`ui/sidebar.py`에 "⭐ 담당구역 즐겨찾기"** 섹션 추가: 저장된 위치 목록(클릭 = 이동, ✕ = 삭제) + 이름 입력 폼("⭐ 현재 위치 저장" — 저장 시점의 `st.session_state.cx/cy`를 그대로 씀).
- 검증: pytest 130 그린(신규 tests/test_favorites_store.py 7개 — 순서 보존, 재저장 갱신, 삭제, 존재하지 않는 이름 삭제는 조용히 무시, 빈 이름 거부, 공백 트리밍). 브라우저 실측 — 강남역에서 "강남역 본진" 저장 → 삼성역 검색 이동(지도가 코엑스·삼성역 권역으로 완전히 바뀜 확인) → 즐겨찾기 클릭으로 강남역 원상 복귀(마커 배치 동일 확인) → ✕ 삭제 후 favorites.json이 빈 객체로 확인. ⚠️ 브라우저 검증 중 Streamlit 세션이 한 번 끊겨(`/_stcore/health` 무응답) 좌표 클릭이 안 먹힌 것처럼 보였던 적이 있음 — 페이지 새로고침으로 재연결, 즐겨찾기는 파일 기반이라 재연결 후에도 그대로 남아 있음을 확인(세션 유실에 영향받지 않는 설계임을 오히려 증명).

▶ 다음 세션 시작점: **적중률(lift) 대시보드** — 노출 로그 + 피드백을 배지별로 묶어 "이 배지가 붙은 추천의 방문가치 없었음 비율 vs 노출 전체 비율" 표를 만든다. n이 작아도(20~30건) 방향은 신뢰 가능 — 이 표를 보고 사람이 weighted_sum의 DEFAULT_WEIGHTS를 수동 조정하는 것이 다음 단계. 표본이 충분히 쌓이면(방문가치 라벨 100건+) 로지스틱 회귀 계수로 대체하는 제4 스코어러(학습 가중치)를 그 다음에 추가. 이후 3단계 챗봇 MVP.
