...
- [x] M4 프랜차이즈 전국 스캔 인프라 (2026-07-07): **영업중 필터(SALS_STTS_CD=01)로 호출량 24,600→7,134회 절감**(전국 영업중 71.3만 — 일반 676,431/단란 11,456/유흥 25,338, 하루 쿼터 내). `datasources/national_names.py` — 페이지 단위 정규화 상호 집계(메모리 미적재)+200페이지 체크포인트 재개. franchise 신호는 전국 카운트 우선(임계 5곳, '전국 N곳 영업 중' 배지)·수집범위 폴백(임계 3) 이중화. ⚠️ 실측 API 속도 ~2.5s/페이지 → 전체 스캔 약 5시간 (nohup 실행 중, 완료 후 써브웨이 케이스 재검증 예정).
