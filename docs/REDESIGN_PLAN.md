...
### Phase 5 — 확장 모델 + 증분 갱신 (진행 중, 2026-07-07)
- [x] `signals/growth_momentum.py`(M3): 500m 격자 개업 가속도(최근 12M − 직전 12M)/활성(하한 5), 기준 명부(자치단체 전체)로 셀 통계 → 유효 셀 percentile이 값. 배지는 상위 20% & 실제 증가 셀만. SGIS 분모 정규화는 키 확보 후 교체 여지.
- [x] `signals/conversion_vector.py`(M6): 자리(주소키)의 직전 폐업 입주자 대비 주류친화도(0~3) 변화. 카페(0)→호프(2) = 값 2/3 + 전환 배지. 반대 방향은 값 0·배지 없음.
- [x] 주소키 강화(outlook.address_key): 말미 (법정동) 주석·쉼표 공백 정규화 — 층·호는 유지(건물 병합은 자리회전 과대 계산). `liquor_affinity()` 점수표·`grid_cell_ids()` 헬퍼 신설.
- [x] 증분 갱신 운영화: `build_index --update-all` (보유 전 파티션 일괄 증분).
- [x] ~~datasources/mfds.py~~ — 행안부 API 일간 갱신으로 불필요 확정 (Phase 0에서 보류 결정).
- [x] 아키텍처 시험 통과: 신호 2개 추가 = 파일 2개 + 등록 import 2줄 + 가중치 2항목 (기존 로직 무수정). 테스트 80개 그린.
- [x] `night_index.py`(M7 v1, 2026-07-07): 값 = 주변 300m 명시적 주류 업태(affinity≥2 — 한식 등 affinity 1을 세면 전부 주류친화가 돼 변별력 소멸) 비중 백분위 × 행정동 심야(23~04시) 생활인구/전일 평균. 좌표→행정동은 VWorld 리버스 지오코딩(level4AC 앞 8자리), 생활인구는 `datasources/seoul.py`(SPOP_LOCAL_RESD_DONG, 게시 지연 대응 14일 전부터 탐색, 캐시 7일). 서울 밖은 우아한 강등(빈 결과). `.env`에 SEOUL_OPEN_DATA_KEY 추가됨. 강남역 실측: 역삼1동 심야 56%, 배지 92곳. 지하철 심야 승하차 축은 역 좌표 매핑 필요로 v1.1 보류.
- [x] M4 프랜차이즈 전국 빈도 (2026-07-07 API 인프라 → 2026-07-11 CSV로 전환·완료): `datasources/national_names.py`.
  - **CSV 경로가 주 경로 (2026-07-11 결정·완료)**: API 전국 스캔(6,700+ 호출·5시간)이 네트워크 끊김으로 세 번 죽어(41%까지) — 전국 프랜차이즈 빈도는 거의 정적(몇 달 1회 갱신)이라 공공데이터포털 전국 인허가 CSV(LOCALDATA 형식, cp949, `data/permit_all.csv` 228만 행) 1회 집계로 대체. `scan_from_csv()`가 영업상태코드=01(영업중 676,390건, API 실측 676,431과 일치)만 chunk 스트리밍 집계 → **30초**에 고유 상호 478,434개. 출력(`national_name_counts.parquet`+meta)은 API 경로와 동일해 franchise 신호 무수정. 실행: `python -m datasources.national_names --csv data/permit_all.csv`.
  - API 스캔 경로(`scan()`)는 폴백으로 유지(재시도 6→30회 강화). franchise 신호는 전국 카운트 우선(임계 5곳, '전국 N곳 영업 중')·수집범위 폴백(임계 3).
  - 실검증: 김밥천국 695·투다리 443·교촌치킨 281 등 대형 체인 정확 카운트, 강남구 453곳 중 체인 배지 54곳. ⚠️ 한계(기존 normalize 한계, CSV 무관): 써브웨이·스타벅스처럼 지점명에 지역이 붙는 체인은 '써브웨이강남'/'써브웨이역삼'으로 흩어져 미검출 — 지명 사전 필요, 점수 미반영이라 실해 작음.
