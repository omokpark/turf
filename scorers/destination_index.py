"""숨은 맛집 (구 목적지 지수, M1 Naver판) — DI = percentile(리뷰 모멘텀 R ÷ 입지 기대치 E)
...
class DestinationIndex:
    ...
    label = "숨은 맛집"
    caption = "입지에 비해 입소문이 유난히 강한 집 — 골목·이면도로의 발굴 후보"
    description = (
        "입지(주변 업소 밀집도)에 비해 블로그 입소문이 유난히 강한 집을 찾습니다. "
        "점수는 '비슷한 조건 업소들의 기대치 대비 몇 배인지'의 구역 내 백분위입니다. "
        "블로그가 한 건도 없는 업소는 제외되며, 어떤 비교군과 비교했는지를 각 배지에 함께 표시합니다. "
        "블로그 수는 체험단·광고로 부풀 수 있습니다 — 건수·시점은 관측 사실 그대로입니다."
    )
