"""방문 타이밍 (구 가중합 베이스라인) 스코어러 — 가용 신호들의 값(0~1)을 가중 평균해 점수를 만든다.
...
class WeightedSum:
    id = "weighted_sum"
    label = "방문 타이밍"
    caption = "신규 개업(공급사 미확정) 최우선의 종합 점수"
    description = (
        "여러 신호를 종합하되 '개업 직후 = 주류 공급사가 아직 정해지지 않은 시점'을 "
        "가장 크게 반영합니다. 업소에 없는 신호는 자동으로 빠지고 나머지 가중치로 재정규화됩니다."
    )
